...
training<-read.csv(file = "pml-trainingMod.csv",
...
modFit.rpart<-train(classe~total_accel_dumbbell+total_accel_arm+total_accel_belt,method="rpart",data=trainingSub)
...
predictions.rpart<-predict(modFit.rpart,newdata=testingSub)
```

```{r model fitting Random Forest}

#modFit.rf<-train(classe~total_accel_dumbbell+total#_accel_arm+total_accel_belt,data=training,method="#rf",prox=TRUE)
